...
db.exec('DELETE FROM manutencao');
db.exec('DELETE FROM links');
db.exec('DELETE FROM links_chamados');
db.exec("UPDATE contadores SET ultimo = 0");
...
const insertCompra = db.prepare(`
  ...
  VALUES (@id, @numero, @desc, @solicitante, @depto, @valor, @status, @prazo, @prioridade, @obs, @data_abertura, @data_conclusao)
`);
compras.forEach(c => insertCompra.run(c));
db.prepare("UPDATE contadores SET ultimo = 7 WHERE modulo = 'compras'").run();
...
const insertManut = db.prepare(`
  ...
  VALUES (@id, @numero, @desc, @local, @tipo, @resp, @status, @sla, @prioridade, @data_abertura, @data_conclusao)
`);
manutencao.forEach(m => insertManut.run(m));
db.prepare("UPDATE contadores SET ultimo = 6 WHERE modulo = 'manutencao'").run();
...
const insertLink = db.prepare(`
  ...
  VALUES (@id, @nome, @tipo, @ip, @velocidade, @latencia, @uptime, @status, @ultima)
`);
